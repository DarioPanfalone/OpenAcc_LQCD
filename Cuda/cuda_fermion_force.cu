...
#ifdef MAGN
magn_angle = tex1Dfetch(magn_texRef,idx+magn_offset+mu*size_dev);
...
sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
       ...
       Mferm_in_0.x=ferm_aux_0.x*cos_magn-ferm_aux_0.y*sin_magn; // Re(ferm_aux_0*e^{i magn phase})
       Mferm_in_0.y=ferm_aux_0.x*sin_magn+ferm_aux_0.y*cos_magn; // Im(ferm_aux_0*e^{i magn phase})

       Mferm_in_1.x=ferm_aux_1.x*cos_magn-ferm_aux_1.y*sin_magn; // Re(ferm_aux_1*e^{i magn phase})
       Mferm_in_1.y=ferm_aux_1.x*sin_magn+ferm_aux_1.y*cos_magn; // Im(ferm_aux_1*e^{i magn phase})

       Mferm_in_2.x=ferm_aux_2.x*cos_magn-ferm_aux_2.y*sin_magn; // Re(ferm_aux_2*e^{i magn phase})
       Mferm_in_2.y=ferm_aux_2.x*sin_magn+ferm_aux_2.y*cos_magn; // Im(ferm_aux_2*e^{i magn phase})
     #endif
...
sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
       ...
       Mferm_in_2.y=ferm_aux_2.x*sin_magn+ferm_aux_2.y*cos_magn; // Im(ferm_aux_2*e^{i magn phase})
    #else
       Mferm_in_0.x=ferm_aux_0.x*dev_eim_cos_f*cos_magn-ferm_aux_0.y*dev_eim_sin_f*sin_magn; // Re(ferm_aux_0*e^{imu})
       Mferm_in_0.y=ferm_aux_0.x*dev_eim_sin_f*sin_magn+ferm_aux_0.y*dev_eim_cos_f*cos_magn; // Im(ferm_aux_0*e^{imu})

       Mferm_in_1.x=ferm_aux_1.x*dev_eim_cos_f*cos_magn-ferm_aux_1.y*dev_eim_sin_f*sin_magn; // Re(ferm_aux_1*e^{imu})
       Mferm_in_1.y=ferm_aux_1.x*dev_eim_sin_f*sin_magn+ferm_aux_1.y*dev_eim_cos_f*cos_magn; // Im(ferm_aux_1*e^{imu})

       Mferm_in_2.x=ferm_aux_2.x*dev_eim_cos_f*cos_magn-ferm_aux_2.y*dev_eim_sin_f*sin_magn; // Re(ferm_aux_2*e^{imu})
       Mferm_in_2.y=ferm_aux_2.x*dev_eim_sin_f*sin_magn+ferm_aux_2.y*dev_eim_cos_f*cos_magn; // Im(ferm_aux_2*e^{imu})
    #endif   
   #endif
...
#ifdef MAGN
magn_angle = tex1Dfetch(magn_texRef,idx + magn_offset + mu*size_dev);
//magn_angle = tex1Dfetch(magn_texRef, 0);
cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
#endif
       ...
       ferm_in_0.x=ferm_aux_0.x*cos_magn-ferm_aux_0.y*sin_magn; // Re(ferm_aux_0*e^{i magn phase})
       ferm_in_0.y=ferm_aux_0.x*sin_magn+ferm_aux_0.y*cos_magn; // Im(ferm_aux_0*e^{i magn phase})

       ferm_in_1.x=ferm_aux_1.x*cos_magn-ferm_aux_1.y*sin_magn; // Re(ferm_aux_1*e^{i magn phase})
       ferm_in_1.y=ferm_aux_1.x*sin_magn+ferm_aux_1.y*cos_magn; // Im(ferm_aux_1*e^{i magn phase})

       ferm_in_2.x=ferm_aux_2.x*cos_magn-ferm_aux_2.y*sin_magn; // Re(ferm_aux_2*e^{i magn phase})
       ferm_in_2.y=ferm_aux_2.x*sin_magn+ferm_aux_2.y*cos_magn; // Im(ferm_aux_2*e^{i magn phase})
     #endif
...
sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
       ...
       ferm_in_2.y=ferm_aux_2.x*sin_magn+ferm_aux_2.y*cos_magn; // Im(ferm_aux_2*e^{i magn phase})
     #else
       ferm_in_0.x=ferm_aux_0.x*dev_eim_cos_f*cos_magn-ferm_aux_0.y*dev_eim_sin_f*sin_magn; // Re(ferm_aux_0*e^{i magn phase}*e^{imu})
       ferm_in_0.y=ferm_aux_0.x*dev_eim_sin_f*sin_magn+ferm_aux_0.y*dev_eim_cos_f*cos_magn; // Im(ferm_aux_0*e^{i magn phase})

       ferm_in_1.x=ferm_aux_1.x*dev_eim_cos_f*cos_magn-ferm_aux_1.y*dev_eim_sin_f*sin_magn; // Re(ferm_aux_1*e^{i magn phase})
       ferm_in_1.y=ferm_aux_1.x*dev_eim_sin_f*sin_magn+ferm_aux_1.y*dev_eim_cos_f*cos_magn; // Im(ferm_aux_1*e^{i magn phase})

       ferm_in_2.x=ferm_aux_2.x*dev_eim_cos_f*cos_magn-ferm_aux_2.y*dev_eim_sin_f*sin_magn; // Re(ferm_aux_2*e^{i magn phase})
       ferm_in_2.y=ferm_aux_2.x*dev_eim_sin_f*sin_magn+ferm_aux_2.y*dev_eim_cos_f*cos_magn; // Im(ferm_aux_2*e^{i magn phase})
     #endif
    #endif
     // ris=ferm^~Mferm
     aux_l[idx +              9*mu*size_dev].x-=shift_coeff[iter]*( ferm_in_0.x*Mferm_in_0.x + ferm_in_0.y*Mferm_in_0.y);
...
__global__ void UmulTAKernel(int *phases,
  ...
  LoadLinkRegs(gauge_texRef, size_dev, idx + gauge_offset, blockIdx.y);    // blockIdx.y corrisponde alla direzione in cui è orientato il link (blockIdx.y=mu=0,1,2,3)
...
// dopo aver calcolato (nelle righe appena precedenti) il prodotto di U per il pezzo calcolato negli step precedenti (product even/odd kernel) viene presa la parte TA.
...
extern "C" void cuda_fermion_force(int num_shifts, 
  ...
  for(ps=0; ps<no_ps; ps++)
     {
     ps_offset=3*size*ps;    // size perche sulla gpu i fermioni "occupano" sia siti pari (col valore che hanno sulla cpu) sia siti dispari (li c'è 0 o viene utilizzato per conservare la parte even quando si moltiplica per Doe ma si vuole anche aggiungere il pezzo con la massa che è proporzionale al fermione originale)

     for(iter=0; iter<num_shifts; iter++)
        {
        CuDoe(Mf, smf_device+num_shifts*ps_offset+3*iter*size, ps, offset_b);   // smf_device corrisponde a loc_s e Mf a loc_h


        FermionProductEvenKernel<<<GridDim, BlockDim>>>(smf_device+num_shifts*ps_offset+3*iter*size,    // qui dentro voglio anche moltiplicare per le fasi magnetiche
                                                  Mf, device_table, device_phases, iter, aux_dev, ps, offset_b);


        cudaCheckError(AT,"FermionProductEvenKernel"); 

        FermionProductOddKernel<<<GridDim, BlockDim>>>(smf_device+num_shifts*ps_offset+3*iter*size,     // qui dentro voglio anche moltiplicare per le fasi magnetiche
                                                  Mf, device_table, device_phases, iter, aux_dev, ps, offset_b);
  ...
  dim3 GridDimBis(size/BlockDimBis.x, 4);  // fa una griglia a due componenti, la seconda dovrebbero essere le direzioni spaziali dei link
  ...
  UmulTAKernel<<<GridDimBis, BlockDimBis>>>(device_phases, aux_dev, ipdot_device, offset_g);   // qui le fasi magnetiche sono già moltiplicate ---> dovrebbe essere tutto ok...
