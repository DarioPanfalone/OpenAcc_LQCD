...
void InitializeShiftVarsD(double *bs, 
  ...
  dim3 ShiftsBlock(NUM_THREADS);
  dim3 ShiftsGrid(size/(2*ShiftsBlock.x),0);
...
void UpdatePsD(double *a, 
  ...
  dim3 UpdatePsGrid(size/(2*UpdatePsBlock.x), 0);
...
void NewShiftVarsD(double *bs, 
  ...
  double *z_out;
  cudaSafe(AT,cudaMalloc((void**)&z_out ,2*sizeof(double)), "cudaSafe");
  ...
  cudaSafe(AT,cudaMemcpy(z, z_out, 2*sizeof(double), cudaMemcpyDeviceToDevice), "cudaMemcpy");
...
extern "C" void cuda_inverter_d(const double residual, 
  ...
  cudaSafe(AT,cudaMalloc((void**)&p          ,  vector_size_d), "cudaMalloc");
  ...
  cudaSafe(AT,cudaMalloc((void**)&bs         ,  sizeof(double)), "cudaMalloc");
  ...
  cudaSafe(AT,cudaMalloc((void**)&z          ,2*sizeof(double)), "cudaMalloc");
  cudaSafe(AT,cudaMalloc((void**)&iz         ,  sizeof(int)), "cudaMalloc");
  cudaSafe(AT,cudaMalloc((void**)&check_conv ,  sizeof(int)), "cudaMalloc");
  ...
  // constants
  cudaSafe(AT,cudaMemcpyToSymbol(residuals_d, res_sq, sizeof(double), 0, cudaMemcpyHostToDevice), 
                                                                           "cudaMemcpyToSymbol");

  // initialize to 0 smf_device
  // 1st float(s)
  cudaSafe(AT,cudaMemset(smf_device , 0, vector_size_f), "cudaMemset");
  // 2nd float(s)
  cudaSafe(AT,cudaMemset(smf_device + offset_2f, 0, vector_size_f), "cudaMemset"); 

  // initialize check counters
  cudaSafe(AT,cudaMemset(check_conv, 0, sizeof(int)), "cudaMemset");
  cudaSafe(AT,cudaMemcpy(iz, &iter_iz, sizeof(int), cudaMemcpyHostToDevice), "cudaMemcpy");

  // r[0]=chi                   
  InitR(r, mf_device , 3*size);
  ...
  // p[num_shifts] := chi
    cudaSafe(AT,cudaMemcpy(p, r, vector_size_d, cudaMemcpyDeviceToDevice), "cudaMemcpy");
  ...
  //  psi[1] -= b[0] p[0]
  InitializeShiftVarsD(bs, z, b, p_0, smf_device, offset_2f);
  ...
  for (k=1; k < max_cg && !conv ; ++k) 
   {
    ...
    UpdatePsD(a, c, cp, z, bs, b, p_0, r, p, check_conv, iz);
    ...
    NewShiftVarsD(bs, bp, z, b, a, c, smf_device, offset_2f, p, check_conv, iz);

    cudaSafe(AT,cudaMemcpy(check_conv_host, check_conv, sizeof(int), cudaMemcpyDeviceToHost), "cudaMemcpy");

    conv = 1;
    conv &=check_conv_host;  // bitwise "and" operator
  ...
  // copy back solution to host
     // 1st float
     cudaSafe(AT,cudaMemcpy(psi_packed , 
                            smf_device , 
                                                 sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
     cudaSafe(AT,cudaMemcpy(psi_packed + size, 
                            smf_device + size, 
                                                 sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
     cudaSafe(AT,cudaMemcpy(psi_packed + 2*size, 
                            smf_device + 2*size, 
                                                 sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
     // 2nd float
     cudaSafe(AT,cudaMemcpy(psi_packed + offset_2f, 
                            smf_device + offset_2f, 
                                                 sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
     cudaSafe(AT,cudaMemcpy(psi_packed + size + offset_2f, 
                            smf_device + size + offset_2f, 
                                                 sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
     cudaSafe(AT,cudaMemcpy(psi_packed + 2*size+ offset_2f, 
                            smf_device + 2*size+ offset_2f, 
                                                 sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");

  // copy back rhs to host
  // 1st float
  cudaSafe(AT,cudaMemcpy(chi_packed, 
                         mf_device ,          sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
  cudaSafe(AT,cudaMemcpy(chi_packed + size, 
                         mf_device + size, sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
  cudaSafe(AT,cudaMemcpy(chi_packed + 2*size, 
                         mf_device + 2*size, sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
  // 2nd float
  cudaSafe(AT,cudaMemcpy(chi_packed + 3*size, 
            mf_device + 3*size, sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
  cudaSafe(AT,cudaMemcpy(chi_packed +  size + 3*size, 
            mf_device  +   size + 3*size, sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
  cudaSafe(AT,cudaMemcpy(chi_packed + 2*size + 3*size, 
            mf_device + 2*size + 3*size, sizeh*sizeof(float2), cudaMemcpyDeviceToHost), "cudaMemcpy");
  #endif
