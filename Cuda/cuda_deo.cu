...
float sin_magn;
...
#ifdef MAGN
  magn_angle = tex1Dfetch(magn_texRef, idx + magn_offset);
  ...
  ferm_out[0][0][threadIdx.x] = ferm_aux_0.x*cos_magn - ferm_aux_0.y*sin_magn;  // Re[e^{i magn phase}*ferm_aux_0] 
  ferm_out[0][1][threadIdx.x] = ferm_aux_0.x*sin_magn + ferm_aux_0.y*cos_magn;  // Im[e^{i magn phase}*ferm_aux_0]

  ferm_out[1][0][threadIdx.x] = ferm_aux_1.x*cos_magn - ferm_aux_1.y*sin_magn;  // Re[e^{i magn phase}*ferm_aux_1]
  ferm_out[1][1][threadIdx.x] = ferm_aux_1.x*sin_magn + ferm_aux_1.y*cos_magn;  // Im[e^{i magn phase}*ferm_aux_1]

  ferm_out[2][0][threadIdx.x] = ferm_aux_2.x*cos_magn - ferm_aux_2.y*sin_magn;  // Re[e^{i magn phase}*ferm_aux_2]
  ferm_out[2][1][threadIdx.x] = ferm_aux_2.x*sin_magn + ferm_aux_2.y*cos_magn;  // Im[e^{i magn phase}*ferm_aux_2]
...
#ifdef MAGN
  magn_angle = tex1Dfetch(magn_texRef, idx + magn_offset + size_dev);
  cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
  sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
#endif
...
#ifdef MAGN
  magn_angle = tex1Dfetch(magn_texRef, idx + magn_offset + 2*size_dev);
  cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
  sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
#endif
 ...
 #ifdef MAGN
  magn_angle = tex1Dfetch(magn_texRef, idx + magn_offset + 3*size_dev);
  cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
  sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
#endif
 ...
 #else
  ferm_out[0][0][threadIdx.x] += ferm_aux_0.x*dev_eim_cos_f*cos_magn - ferm_aux_0.y*dev_eim_sin_f*sin_magn;  // Re[e^{imu}*e^{i magn phase}*ferm_aux_0]
  ferm_out[0][1][threadIdx.x] += ferm_aux_0.x*dev_eim_sin_f*sin_magn + ferm_aux_0.y*dev_eim_cos_f*cos_magn;  // Im[e^{imu}*e^{i magn phase}*ferm_aux_0]

  ferm_out[1][0][threadIdx.x] += ferm_aux_1.x*dev_eim_cos_f*cos_magn - ferm_aux_1.y*dev_eim_sin_f*sin_magn;  // Re[e^{imu}*e^{i magn phase}*ferm_aux_1]
  ferm_out[1][1][threadIdx.x] += ferm_aux_1.x*dev_eim_sin_f*sin_magn + ferm_aux_1.y*dev_eim_cos_f*cos_magn;  // Im[e^{imu}*e^{i magn phase}*ferm_aux_1]

  ferm_out[2][0][threadIdx.x] += ferm_aux_2.x*dev_eim_cos_f*cos_magn - ferm_aux_2.y*dev_eim_sin_f*sin_magn;  // Re[e^{imu}*e^{i magn phase}*ferm_aux_2]
  ferm_out[2][1][threadIdx.x] += ferm_aux_2.x*dev_eim_sin_f*sin_magn + ferm_aux_2.y*dev_eim_cos_f*cos_magn;  // Im[e^{imu}*e^{i magn phase}*ferm_aux_2]
 #endif
#endif
...
#ifdef MAGN
  magn_angle = tex1Dfetch(magn_texRef, site_table[threadIdx.x] + magn_offset);
  cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
  sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
#endif
...
#ifdef MAGN
  magn_angle = tex1Dfetch(magn_texRef, site_table[threadIdx.x] + magn_offset + size_dev);
  cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
  sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
#endif
  ...
  #ifdef MAGN
    magn_angle = tex1Dfetch(magn_texRef, site_table[threadIdx.x] + magn_offset + 2*size_dev);
    cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
    sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
  #endif  
 ...
 #ifdef MAGN
    magn_angle = tex1Dfetch(magn_texRef, site_table[threadIdx.x] + magn_offset + 3*size_dev);
    cos_magn = cosf(device_charges_f[ps_ferm]*magn_angle);
    sin_magn = sinf(device_charges_f[ps_ferm]*magn_angle);
 #endif 
...
void CuDoe(float2 *out, 
  ...
  DoeKernel<<<GridDimension,BlockDimension>>>(in, out, device_table, device_phases, offset_g, offset_f, offset_b, ps_ferm); 
