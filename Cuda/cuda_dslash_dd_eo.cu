#ifndef _CUDA_DSLASH_DD
#define _CUDA_DSLASH_DD

// DOUBLE PRECISION KERNEL for even/odd fermions

__global__ void switchParityKernel(double2 *out,
                                     double2 *in)
{ 
...
__global__ void copyKernel(double2 *out,
                            double2 *in)
{ 
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, idx+magn_offset);
   magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
#endif
  ...
  ferm_aux_2.x = C1RED*ferm_in_0.x-C1IMD*ferm_in_0.y+
	         C2RED*ferm_in_1.x-C2IMD*ferm_in_1.y+
	         C3RED*ferm_in_2.x-C3IMD*ferm_in_2.y;
  ferm_aux_2.y = C1RED*ferm_in_0.y+C1IMD*ferm_in_0.x+
	         C2RED*ferm_in_1.y+C2IMD*ferm_in_1.x+
	         C3RED*ferm_in_2.y+C3IMD*ferm_in_2.x;
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, idx+magn_offset+size_dev);
  magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, idx+magn_offset+2*size_dev);
  magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, idx+magn_offset+3*size_dev);
  magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
  ...
  #else
  ferm_out[0][0][threadIdx.x] += ferm_aux_0.x*dev_eim_cos_d*cos_magn - ferm_aux_0.y*dev_eim_sin_d*sin_magn;  // Re[e^{imu}*e^{i charge field}*ferm_aux_0]
  ferm_out[0][1][threadIdx.x] += ferm_aux_0.x*dev_eim_sin_d*sin_magn + ferm_aux_0.y*dev_eim_cos_d*cos_magn;  // Im[e^{imu}*e^{i charge field}*ferm_aux_0]

  ferm_out[1][0][threadIdx.x] += ferm_aux_1.x*dev_eim_cos_d*cos_magn - ferm_aux_1.y*dev_eim_sin_d*sin_magn;  // Re[e^{imu}*e^{i charge field}*ferm_aux_1]
  ferm_out[1][1][threadIdx.x] += ferm_aux_1.x*dev_eim_sin_d*sin_magn + ferm_aux_1.y*dev_eim_cos_d*cos_magn;  // Im[e^{imu}*e^{i charge field}*ferm_aux_1]

  ferm_out[2][0][threadIdx.x] += ferm_aux_2.x*dev_eim_cos_d*cos_magn - ferm_aux_2.y*dev_eim_sin_d*sin_magn;  // Re[e^{imu}*e^{i charge field}*ferm_aux_2]
  ferm_out[2][1][threadIdx.x] += ferm_aux_2.x*dev_eim_sin_d*sin_magn + ferm_aux_2.y*dev_eim_cos_d*cos_magn;  // Im[e^{imu}*e^{i charge field}*ferm_aux_2]
  #endif
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, site_table[threadIdx.x]+magn_offset);
  magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, site_table[threadIdx.x]+magn_offset+size_dev);
  magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, site_table[threadIdx.x]+magn_offset+2*size_dev);
  magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
...
#ifdef MAGN
   aux_angle   = tex1Dfetch(magn_texRef, site_table[threadIdx.x]+magn_offset+3*size_dev);
  magn_angle  = (double)aux_angle;
//  magn_angle=(double)0.0;
  sin_magn=sin(magn_angle*device_charges_d[ps_ferm]);  // uscirà come double???
  cos_magn=cos(magn_angle*device_charges_d[ps_ferm]);
...
void switchParity(double2 *out, double2* in)
{
...
void copy(double2 *out, double2* in)
{
