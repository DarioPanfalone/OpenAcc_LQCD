...
static inline void combine_fourleaves_to_get_loc_q(__restrict su3_soa * const Qmn, int idx_Qmn,
  ...
  locq->d[idx_locq] += creal(A00) * Eps / (256.0 * M_PI * M_PI);
...
double compute_topological_charge(__restrict su3_soa * const u,
  ...
  compute_local_topological_charge(u,quadri,loc_q);
  mult_conf_times_stag_phases(u);
#pragma acc update device(loc_q[0:2])
  print_1double_soa(&loc_q[0],"carica_even");
  print_1double_soa(&loc_q[1],"carica_odd");
  double res =reduce_loc_top_charge(loc_q);
  return  res;
}
