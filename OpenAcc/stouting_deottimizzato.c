...
#pragma acc routine seq
static inline void single_su3_to_su3_soa(__restrict su3_soa * const out,
        int idx,
        single_su3 * in){
    out->r0.c0[idx]  = in->comp[0][0];
    out->r0.c1[idx]  = in->comp[0][1];
    out->r0.c2[idx]  = in->comp[0][2];
    out->r1.c0[idx]  = in->comp[1][0];
    out->r1.c1[idx]  = in->comp[1][1];
    out->r1.c2[idx]  = in->comp[1][2];
}
...
#pragma acc routine seq
static inline void single_tamat_to_tamat_soa( __restrict tamat_soa * const out,
					      int idx,
					      single_tamat * in){
  in->c01[idx] = out->c01 ;
  in->c02[idx] = out->c02 ;
  in->c12[idx] = out->c12 ;
  in->rc00[idx] = out->rc00;
  in->rc11[idx] = out->rc11;

}

#pragma acc routine seq
inline void single_su3_CeqAB(single_su3 *C , single_su3 * A , single_su3 *B){
