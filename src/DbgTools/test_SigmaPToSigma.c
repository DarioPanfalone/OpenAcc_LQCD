...
int main(){
  ...
  conf_hasenbusch[0]->status = IN_USE;
...
#pragma acc data create(aux_th[0:8]) create(aux_ta[0:8]) create(gstout_conf_acc_arr[0:(8*STOUT_STEPS)]) create(glocal_staples[0:8]) create(gipdot[0:8]) 
    ...
    for(int id_iter=id_iter_offset;id_iter<(ITERATIONS+id_iter_offset);id_iter++){
		 ...
		 accettate_therm = UPDATE_SOLOACC_UNOSTEP_VERSATILE(conf_hasenbusch[0],
                 residue_metro,residue_md,id_iter-id_iter_offset,
                 accettate_therm,0);
	       }else{
		 accettate_metro = UPDATE_SOLOACC_UNOSTEP_VERSATILE(conf_hasenbusch[0],residue_metro,residue_md,id_iter-id_iter_offset-accettate_therm,accettate_metro,1);
	       }
#pragma acc update host(conf_hasenbusch[0][0:8])
	       //---------------------------------------//
		 ...
		 for(int iflv=0;iflv<NDiffFlavs;iflv++) perform_chiral_measures(conf_hasenbusch[0],u1_back_field_phases,&(fermions_parameters[iflv]),residue_metro,foutfile);
		 fprintf(foutfile,"\n");
	       }
	       fclose(foutfile);
	       //-------------------------------------------------// 
		 ...
		 if(id_iter<therm_ITERATIONS){
	       ...
	       if(conf_id_iter%save_conf_every==0)	print_su3_soa(conf_hasenbusch[0],"stored_config");
	       //-------------------------------------------------//
    ...
    //--------- SALVA LA CONF SU FILE ------------------//
    print_su3_soa(conf_hasenbusch[0],"stored_config");
    //-------------------------------------------------//
