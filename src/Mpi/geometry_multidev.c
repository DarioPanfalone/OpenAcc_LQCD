...
void setup_nnranks(int **nnranks, int myrank){
    ...
    mdevinfo.myrank4int = xyzt_rank(myrank);
    int myrank_vect[4];
    myrank_vect[0] = mdevinfo.myrank4int.d0 ;
    myrank_vect[1] = mdevinfo.myrank4int.d1 ; 
    myrank_vect[2] = mdevinfo.myrank4int.d2 ; 
    myrank_vect[3] = mdevinfo.myrank4int.d3 ; 
    int dir;
    for(dir=0;dir<4;dir++){
