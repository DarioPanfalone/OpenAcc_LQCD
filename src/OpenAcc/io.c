...
int read_su3_soa_ASCII(su3_soa * conf, const char* nomefile,int * conf_id_iter ){
  ...
  for(int q = 0 ; q < 8 ; q++){
    for(int i = 0 ; i < sizeh ; i++){
      double re,im;
      //      fscanf(fp, "%.18lf\t%.18lf\n",&re,&im);
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r0.c0[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r0.c1[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r0.c2[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r1.c0[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r1.c1[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r1.c2[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r2.c0[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r2.c1[i] = re + im * I;
      CHECKREAD(fscanf(fp, "%lf %lf\n",&re,&im),2);conf[q].r2.c2[i] = re + im * I;
